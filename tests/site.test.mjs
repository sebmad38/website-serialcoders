...
import { buyerServicePages } from '../src/pages/buyer-services.mjs';
const expectedPageCount = 10 + buyerServicePages.length;

const source = await readFile(outputPath('site.js'), 'utf8');
...
function harness({ id = 'G-TEST123', stored = null, unavailable = false } = {}) {
  ...
  const links = [
    element({ event: 'contact_email_click' }),
    element({ event: 'contact_phone_click' }),
  ];
  ...
  const document = {
    ...
    getElementById: (n) => (n === 'consent' ? banner : settings),
    querySelectorAll: () => links,
    createElement: () => ({}),
    head: { append: (s) => appended.push(s) },
  };
  const localStorage = {
    getItem() {
  ...
  vm.runInNewContext(source, {
    ...
    localStorage,
    location: {
      origin: 'https://serialcoders.fr',
      pathname: '/contact/',
      hostname: 'serialcoders.fr',
    },
    Date,
    encodeURIComponent,
  });
...
test('No Google request before consent or after refusal', () => {
  ...
  h.buttons[0].events.click();
  h.links[0].events.click();
  assert.equal(h.appended.length, 0);
...
test('Consent loads one tag, records contacts, and withdrawal stops events', () => {
  ...
  h.links[0].events.click();
  assert.equal(commands().at(-1)[1], 'contact_email_click');
  h.buttons[0].events.click();
...
test('Expired consent is not silently reused', () => {
  const h = harness({ stored: JSON.stringify({ value: 'granted', expires: Date.now() - 1000 }) });
  assert.equal(h.appended.length, 0);
  assert.equal(h.banner.hidden, false);
});
...
test('Saved refusal stays silent', () => {
  const h = harness({ stored: JSON.stringify({ value: 'denied', expires: Date.now() + 60000 }) });
  assert.equal(h.appended.length, 0);
  assert.equal(h.banner.hidden, true);
});
test('All pages have unique SEO metadata and resolvable internal links', async () => {
  ...
  const htmlFiles = files.filter((p) => p.endsWith('.html') && p !== '404.html');
  ...
  for (const file of htmlFiles) {
    ...
    assert.equal(
      html.match(/<meta\s+name="robots"\s+content="([^"]+)"/)[1],
      config.production ? 'index, follow' : 'noindex, nofollow',
    );
    assert.ok(!html.includes('href="#"'));
    for (const match of html.matchAll(/(?:href|src)="(\/[^"#]*)(?:#[^"]*)?"/g)) {
      const path = new URL(match[1], config.origin).pathname;
      await access(resolve(outputDirectory, `.${path}${path.endsWith('/') ? 'index.html' : ''}`));
    }
    const json = html.match(/<script type="application\/ld\+json">(.*?)<\/script>/s)[1];
    assert.equal(JSON.parse(json)['@type'], 'Organization');
  }
});
test('Robots and sitemap match the release mode and canonical URLs', async () => {
  ...
  for (const url of urls) {
    ...
    const html = await readFile(
      resolve(outputDirectory, `.${new URL(url).pathname}index.html`),
      'utf8',
    );
    assert.equal(html.match(/<link\s+rel="canonical"\s+href="([^"]+)"/)[1], url);
  }
  assert.match(await readFile(outputPath('404.html'), 'utf8'), /name="robots"\s+content="noindex"/);
});

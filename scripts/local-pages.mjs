...
/** A navigable service directory, without fabricated premises or local testimonials. */
export function createLocalPages(dataset) {
  ...
  for (const city of cities) {
    ...
    const department = escapeHtml(city.departement.nom);
    const region = escapeHtml(city.region.nom);

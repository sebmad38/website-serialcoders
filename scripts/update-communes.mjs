...
const source =
  'https://geo.api.gouv.fr/communes?fields=nom,code,population,departement,region,codesPostaux';
...
if (communes.length < 2000)
  throw new Error('Source incomplète : vérifier avant de remplacer le référentiel.');
const dataset = {
  source,
  ...
  missingPopulation: records
    .filter((city) => !Number.isInteger(city.population))
...
console.log(
  `${communes.length} communes importées ; ${dataset.missingPopulation.length} territoires sans population renseignée.`,
);
